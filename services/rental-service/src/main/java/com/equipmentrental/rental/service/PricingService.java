...
public class PricingService {
    ...
    public RentalPriceResponse updatePrice(Long id, RentalPriceRequest r) {
        ...
        if (!e.getOrganizationId().equals(r.organizationId()) || !e.getBranchId().equals(r.branchId())) {
    ...
    public DiscountCodeResponse updateDiscount(Long id, DiscountCodeRequest r) {
        ...
        DiscountCode discount = discounts.findById(id).orElseThrow(() -> ApiException.notFound("Không tìm thấy mã giảm giá"));
        dataScopeGuard.requireBranch(discount.getOrganizationId(), discount.getBranchId());
        if (!discount.getOrganizationId().equals(r.organizationId()) || !discount.getBranchId().equals(r.branchId())) {
            throw new ApiException("Không được thay đổi organizationId hoặc branchId của mã giảm giá");
        }
        discount.setCode(r.code().toUpperCase()); discount.setName(r.name()); discount.setDiscountType(r.discountType());
        discount.setDiscountValue(r.discountValue()); discount.setMaxDiscount(r.maxDiscount());
        discount.setMinOrderValue(r.minOrderValue()); discount.setCustomerGroup(r.customerGroup());
        discount.setValidFrom(r.validFrom()); discount.setValidTo(r.validTo());
    ...
    public BigDecimal calculateDiscount(String code, BigDecimal amount, Long organizationId, Long branchId) {
        ...
        dataScopeGuard.requireBranch(organizationId, branchId);
        DiscountCode d = discounts.findByOrganizationIdAndBranchIdAndCodeIgnoreCase(organizationId, branchId, code)
